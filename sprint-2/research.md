...
**AD-33** _(S9)_ **Q3 is descoped.** The private/org-scoped Alexa+ distribution question stays open — it is a Sprint 3 decision gated on Amazon's policies, and it does not block Sprint 2 closure. E6 closes as "Q2 answered by observation (Auth0 honors RFC 8707), Q3 descoped," rather than staying open indefinitely.
**AD-42** _(S9)_ **The HTTP boundary owns `tools/call` auditing.** One writer, one row per request — including calls the SDK rejects before dispatch (unknown tool, schema-invalid arguments), which handler-side auditing could never see (eval F-1). Handlers enrich the row through `requestMeta.toolAudit`; absent enrichment after dispatch is itself the signal (pre-dispatch rejection). The write follows the response flush, so readers of `mcp_audit` tolerate a few ms of commit lag.
